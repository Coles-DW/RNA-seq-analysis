...
write.table(colData, file="col_Samples_Conditions.txt", sep="\t")
...
hist(as.matrix(log2(data + epsilon)), breaks=100, col="blue", border="white",
     ...
     las=1, cex.axis=0.7)
dev.copy(png,'Log2-transformed_Histogram.png')
dev.off()
...
dev.copy(png,'boxplot_expressed_transformeddata.png')
dev.off()
######dds=data deseq
...
write.table(norm_counts, file="normalized_expressed.txt", sep="\t")
##rlog tranformation
...
dev.copy(png,'Expressed_sampletosample_heatmap.png')
dev.off()
...
write.table(r, file="correlation_values.txt", sep="\t")
## calcul de la dissimilarité ##
d=1-r
## Clustering ##
h = hclust(as.dist(d), method = "complete", members = NULL)
plot(h)
dev.copy(pdf, 'Expressed_hclust.pdf')
dev.off()
...
geneexpressionresults=cbind(res1, res2)
write.table(geneexpressionresults, file="Differentialexpression.txt", sep="\t")
