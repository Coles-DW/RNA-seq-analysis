...
myData <- rlogdata[-c(1, 2, 3, 7, 8, 9, 10, 11, 12, 13, 14, 15, 19, 20, 21), ] #this line tells R which columns to keep and which to throw out

#need a condition vector with the classification of each sample
conditionvector <- as.data.frame(condition)
write.csv(conditionvector, "definegroups.csv")
conditionvector <- read.csv("definegroups.csv")
...
write.table(loadings_rvss, file="pcaloadings.txt", sep="\t")

#extracting the coordinates
coordinates_rvss<-as.data.frame(mypca[["variates"]][["X"]])
write.table(coordinates_rvss, file="pcacoordinates.txt", sep="\t")

myData2 <- t(myData)
write.csv(myData2, "rldcounts.csv")
